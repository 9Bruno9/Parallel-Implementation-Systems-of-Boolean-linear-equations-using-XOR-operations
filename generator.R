...
data_p1 <-read.csv("result_data/risultati_p1_01_20_3050.csv")
...
data_p2 <-read.csv("result_data/risultati_p2_01_20_3050.csv")
data_p2 <-data_p2 %>%
  group_by(n) %>%
  summarise(t_medio = mean(tempo_esecuzione))

data_p3 <-read.csv("result_data/risultati_p3_01_20_3050.csv")
data_p3 <-data_p3 %>%
  group_by(n) %>%
  summarise(t_medio = mean(tempo_esecuzione))
...
ggplot(data_all, aes(x = n^2, y = t_medio, color = tipo)) +
  ...
  labs(
    ...
    title = "Confronto tempi seriale vs p1 vs p2 vs p3 vs p5;\n theta =0.1"
  ) +
...
ggplot(data_all, aes(x = n^2, y = t_medio, color = tipo)) +
  ...
  labs(
    ...
    y = "Tempo medio di esecuzione",
    color = "Tipo esecuzione",
    title = "Confronto tempi p3 vs p5; theta =0.1"
  ) +
  theme_minimal(base_size = 14) # tema pulito
