...
__global__ void rowsElimination(uint8_t* matrix,
                     int n,
                     int k,
                     int pivotRow,
                     int pivotCol)
...
bool gaussianEliminationCuda1(uint8_t* h_matrix, int n, int k, uint8_t* solution)
{
    ...
    CHECK(cudaMemcpy(d_matrix, h_matrix,
                     n*k*sizeof(uint8_t),
    ...
    {
        int pivot = -1;
        // Cerco pivot su CPU (usando copia host)
        ...
        rowsElimination<<<blocks, threads>>>(
            d_matrix, n, k, rank, col);
        CHECK(cudaDeviceSynchronize());

        CHECK(cudaMemcpy(h_matrix, d_matrix,
                         n*k*sizeof(uint8_t),
